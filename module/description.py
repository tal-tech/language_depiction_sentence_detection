...
class Description(object):
    ...
    def init(self, key_list=list(desc_key_map.keys())):
        ...
        try:
            ...
            return -1011005, '描写对象初始化错误:描写语言模型初始化错误'
    ...
    def get_all_descriptions(self, sent_list, essay_type='common'):
        ...
        try:
            ...
            info = {}
            info['language'] =  {'pred_result': [], 'match_result': []}
            ...
            description_info['description'] = "描写模块批处理:语言模块处理错误!"
    ...
    def classify(self, sent_list, key):
     ...
     # 外貌描写
    def get_appearance(self, sent_list):
        match_list, pred_list, proba_list = self.classify(sent_list, 'appearance')
        return match_list, pred_list, len(match_list)

    # 神态描写
    def get_shentai(self, sent_list):
        match_list, pred_list, proba_list = self.classify(sent_list, 'shentai')
        return match_list, pred_list, len(match_list)

    # 心理描写
    def get_psychology(self, sent_list):
        match_list, pred_list, proba_list = self.classify(sent_list, 'psychology')
        return match_list, pred_list, len(match_list)

    # 动作描写
    def get_action(self, sent_list):
        st = time.time()
        match_list, pred_list, proba_list = self.classify(sent_list, 'action')
        # print('action_time '*5, time.time()-st)
        return match_list, pred_list, len(match_list)

    # 环境描写
    def get_environment(self, sent_list):
        match_list, pred_list, proba_list = self.classify(sent_list, 'environment')
        return match_list, pred_list, len(match_list)

    # 景物描写
    def get_scenery(self, sent_list):
        match_list, pred_list, proba_list = self.classify(sent_list, 'scenery')
        return match_list, pred_list, len(match_list)
